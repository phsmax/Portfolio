...
public class Inventory : MonoBehaviour
{
    ...
    public int num;//슬롯증가시 컨텐트사이즈를 조절해줄 변수
    ...
    public void slotup()//인벤토리의 슬롯추가버튼을 눌렀을때 실행되는함수
    {
        num++;//슬롯추가버튼을 눌렀을때 넘이 증가 그만큼 인벤토리의 소지할수있는 아이템이 늘어나게하는 변수
        if (num > 8)//넘이 8보다 크면 계속 8 8번추가가 max
            num = 8;
        ShowItem();//인벤토리 갱신
